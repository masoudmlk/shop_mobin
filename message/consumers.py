...
class ChatConsumer(WebsocketConsumer):
    ...
    def save_info(self, username, message):
        user = User.objects.filter(username=username).first()
        message = Message.objects.create(author=user, content=message, related_group_id=self.group_id)
    ...
    def receive(self, text_data=None, bytes_data=None):
        ...
        if command == self.COMMANDS['new_message']:
            self.save_info(username, message)
    ...
    # if isinstance(message, Message):
        ...
        # user = User.objects.filter(username='mlk').first()
        # message = Message.objects.create(author=user, content=message)
        # print(message)
        # self.save_info(username, message)
